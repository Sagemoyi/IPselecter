...
| `--token` | *(自动生成)* | 订阅路径安全混淆 Token。留空则自动生成并持久化保存极强随机串到 `.token` 文件 |
...
1. **防爆破与公网安全**：为防探测，所有配置文件输出与内置 HTTP 均启用了 Token 目录混淆并关闭了目录一览。初始运行后请查看生成的 `.token` 文件记录你的专属后缀，或使用 `--token` 自行指定。
2. **防火墙**：使用 LAN/公网 服务时，确保你的防火墙允许对应端口（默认 8765）的入站连接。
3. **节点安全**：`节点订阅链接.txt` 和 `vm-nodes.yaml` 包含敏感信息，请勿提交到公开仓库。
4. **操作系统的智能判断**：脚本已支持自动判断系统。如果是 Linux，会自动选取 `cfst_linux_amd64/` 目录；如果是 Windows 则是 `cfst_windows_amd64/`。
5. **CFST 路径**：只需确保对应目录下有正确的平台的 cfst 二进制文件及 ip.txt 即可，通常无需再手动指定 `--cfst-exe` 等参数。
